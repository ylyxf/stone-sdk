...
<%@ attribute name="showTip" type="java.lang.String" %>
...
<c:choose>
    ...
    <c:when test="${type=='select'}">
    	<select  <c:forEach items="${userAttributes}" var="userAttribute" > ${userAttribute.key}="${userAttribute.value}" </c:forEach>>
    		<%-- 是否选中默认值 --%>
    		<c:set var="defaultValueSelected" value="selected" />
    		<%-- 循环出数据库的字典内容 --%>
    		<c:forEach var="dictItem" items="${dictItemList}">
    			<option value="${dictItem.value }"  <c:if test="${value==dictItem.value }"> selected<c:set var="defaultValueSelected" value="" /></c:if> >${dictItem.label }</option>
    		</c:forEach>
    		<%-- 是否显示默认值 --%>
    		<c:if test="${showTip}">
    			<option value="${defaultValue}" ${defaultValueSelected}>请选择---</option>
    		</c:if>
    	</select>
	</c:when> 
	<%-- 单选框 --%>
